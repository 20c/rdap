...
def test_rdap_entity_lookup(rdapc, data_normalize_entity):
    with patch("rdap.normalize.geo.lookup") as mock_lookup:
        ...
        assert json.dumps(data_normalize_entity.expected, indent=2) == json.dumps(entity.normalized, indent=2)
...
def test_rdap_domain_lookup(rdapc, data_normalize_domain):
    ...
    assert json.dumps(data_normalize_domain.expected, indent=2) == json.dumps(entity.normalized, indent=2)
